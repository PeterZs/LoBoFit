...
## Demo
![](ffd_draft_0602.gif) <br />
LoBoFit offers two key advantages. First, it provides a stable initialization, keeping the garment semantically attached to the target skeleton while adapting to different bone proportions and local joint configurations. Second, during refinement, it optimizes localized residuals to resolve collisions and preserve details efficiently, decomposing global deformation into well-conditioned local subproblems for stable optimization. LoBoFit also supports hierarchical coarse-to-fine refinement.
